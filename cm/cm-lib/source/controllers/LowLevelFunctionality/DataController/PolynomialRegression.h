...
public:
    ...
    void printCoeffs(std::vector<double>& coeffs){
        qDebug()<<"\n\nPolynomial Coefficients\n";
        for(std::vector<double>::size_type i = 0; i < coeffs.size(); i ++){
            qDebug() << "a" << i << " = " << coeffs.at(i);
        }
    }

    std::vector<double> getYfromXAndPolynomialCoeffs(std::vector<double>& xSet, std::vector<double>& coeffs, std::vector<double> ySetOriginal = {}){
        std::vector<double> ySet {};
        for(auto x : xSet){
            double y = pow(x,3)*coeffs.at(3) + pow(x,2)*coeffs.at(2) + x*coeffs.at(1) + coeffs.at(0);
            ySet.push_back(y);
        }

        if( not ySetOriginal.empty() ){
            qDebug() << "\nY value\noriginal vs calculated";
            for(std::vector<double>::size_type i = 0; i < ySet.size(); i ++){
                qDebug() <<  ySet.at(i) << "  vs  " << ySetOriginal.at(i);
            }
            qDebug()<<"\n";
        }
